...
Figure~\ref{fig:examples} shows representative $\approx$14\,s windows selected
...
waveform per session (Fig.~\ref{fig:morph}a,b). The session medians were then compared
quantitatively by pairwise correlation (Fig.~\ref{fig:morph}c), and the firing timing
summarised by the coupling-interval distribution (Fig.~\ref{fig:morph}d).
...
trough (Fig.~\ref{fig:morph}b) but still correlates at $r\geq0.99$, so the difference is
...
The high correlation excludes a \emph{second} focus with a distinct QRS, but not two
morphological subtypes hidden inside one continuous-looking cloud (mild bifocality).
Four data-driven checks address that (Figure~\ref{fig:continuum}):
\begin{itemize}
