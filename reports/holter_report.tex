...
Burden $=$ PVCs / all beats; effective SA rate $=$ 60000 /
...
\reportfig{09_cross_session_rhythm_and_burden.png}{(a) PVC burden by session; (b) effective rate vs PVC pause type; (c) interpolated vs compensated composition; (d) pre-PVC coupling-interval stability.\label{fig:cross}}
\tableCross

\textbf{Key pattern.} The single relationship that ties the dataset together appears in panel (b)
of \textbf{Figure~\ref{fig:cross}}: the share of compensated
(felt) PVCs rises with the resting heart rate while interpolated (silent) ones fall (Spearman
$r\approx0.78$, $p\approx0.008$) --- so the number of thumps the subject notices tracks heart
rate, not raw PVC count.
