...
\begin{itemize}
  \item \textbf{PCA + K-means (k=2)} forces a bipartition. Two well-separated
        ...
        middle means the split is an artefact (\textbf{Fig.~\ref{fig:continuum}a}).
  \item \textbf{PVCs sorted by hyperpolarisation depth} --- two sharp bands
        indicate subtypes, a smooth gradient a continuum (\textbf{Fig.~\ref{fig:continuum}b}).
  \item \textbf{Elbow plot} --- a sharp knee would suggest a natural number of
        clusters; a smooth decay, none (\textbf{Fig.~\ref{fig:continuum}c}).
  \item \textbf{Trough-depth histogram} --- one unimodal peak is consistent
        with a single focus; two separated peaks would not be (\textbf{Fig.~\ref{fig:continuum}d}).
\end{itemize}
...
monotonically with no knee; and the trough-depth distribution is unimodal. This pattern is
consistent with a single focus showing continuous beat-to-beat variation, in line with the
