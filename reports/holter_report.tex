...
The cross-session N medians are highly concordant --- all pairwise correlations stay
$r\geq0.93$ (the correlation colour scale starts at $0.95$ to surface any residual drift). One
session is consistently the least similar to the rest: 06-09 13:23, with the lowest mean
correlation to the others ($r=0.962$; \textbf{Fig.~\ref{fig:nmorph}d}). It is also the only
recording made seated at a desk with anterior trunk flexion, while all others were lying /
extended.

A trunk-position change rotates the heart axis relative to the single precordial electrode,
which plausibly explains this morphology drift. The effect surfaces in N but not in PVC because
the PVC's large dominant deflection ($\approx$1.5\,V) stays robust to small projection changes
once amplitude-normalised, whereas the smaller, roughly symmetric N ($\approx$0.5\,V) amplifies
the relative weight of the P- and T-wave regions. This is an interpretation of the data, not a
confirmed mechanism.

\reportfig{04_normal_beat_morphology_4_panel_summary.png}{Normal-beat morphology. (a) all N beats overlaid (amplitude-normalised) with the median and IQR; (b) one median waveform per session; (c) pairwise cross-session correlation matrix (colour: Pearson $r$, scale 0.95--1.0; all $r\geq0.93$); (d) the most divergent session (06-09 13:23, seated / trunk-flexed) median N against the median and IQR of the other nine.\label{fig:nmorph}}
