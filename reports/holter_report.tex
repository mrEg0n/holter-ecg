...
\interp{interpolated} PVC occurs between two sinus beats without producing a substantial delay
of the next sinus beat, so the N--PVC--N interval remains close to one local sinus cycle. A
\comp{compensated} PVC is followed by a longer post-extrasystolic pause, so the N--PVC--N
...
boundary. \textbf{Figure~\ref{fig:strip3}} shows this on a continuous strip, with the clearly
\interp{interpolated} beats in blue, the clearly \comp{compensated} in red, and the borderline
intermediate beats in yellow.

\reportfig{interp_comp_3class_strip.png}{A continuous strip from the same recording, classified by the post-extrasystolic pause \rrpost{}: \interp{interpolated} (short pause, blue), \comp{compensated} (long pause, red), and the borderline beats near the valley (yellow). Most PVCs are unambiguous; only a few sit at the boundary.\label{fig:strip3}}
...
to an \interp{interpolated} PVC, whereas a long \rrpost{} corresponds to a \comp{compensated} PVC
...
sinus cycle. Beats with \rrpost{} below this value were classified as \interp{interpolated}, and
beats above it as \comp{compensated}. Across the dataset this criterion classified
...
Each PVC is coloured according to its class: \interp{interpolated} PVCs are shown in blue and
\comp{compensated} PVCs in red. Interpolated PVCs occur without a substantial gap in the
surrounding sinus rhythm, whereas compensated PVCs are followed by a visibly longer pause and a
stronger post-extrasystolic beat.

\reportfig{08_final_2_class_strip.png}{The same recording classified by the actual pause \rrpost{} (two classes, no grey zone): \interp{interpolated} (blue) carry no pause, \comp{compensated} (red) are followed by one.\label{fig:strip2}}
