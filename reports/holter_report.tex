...
Applying a strict conventional sum rule --- a ``full'' compensatory pause defined as N--PVC--N
between $1.85$ and $2.15\times$ the local sinus cycle --- leaves a broad intermediate range of
beats. Classifying the PVCs by this sum alone, only about 1\% fall in the clean interpolated
band and about 60\% in the clean compensated band, while roughly 39\% land in the intermediate
grey zone in between. \textbf{Figure~\ref{fig:strip3}} shows this on a continuous strip, with the
grey-zone PVCs in yellow.

\reportfig{interp_comp_3class_strip.png}{The same recording as Fig.~\ref{fig:strip2}, classified by the conventional sum S alone: \interp{interpolated} ($S\leq1.15$, blue), \comp{compensated} ($S\geq1.85$, red), and the intermediate grey zone ($1.15<S<1.85$, yellow). With the sum alone a large share of PVCs are ambiguous.\label{fig:strip3}}

The dataset, however, shows a clearer structure than this rule alone suggests.
