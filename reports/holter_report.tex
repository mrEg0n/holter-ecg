...
{\itshape Personal report --- June 2026}
\end{center}
...
Figure~\ref{fig:quality} illustrates the detection and curation procedure on a continuous segment from one recording, displayed as consecutive 12\,s rows. The trace is stable over most of the segment, and each detected beat carries its automatic label, with PVCs shown in \comp{red} and sinus beats in \textcolor{ForestGreen}{green}. A single shaded interval corresponds to a burst of motion noise and is visually distinct from the surrounding ECG. This is the type of segment removed during manual noise curation.

\reportfigP{quality_strip.png}{A continuous 72-second stretch of one session, in consecutive 12-second rows. The ECG is clean and every beat is detected automatically (PVCs in red, sinus beats in green); the shaded 10-second stretch is motion / contact noise, marked by hand and excluded before analysis --- no detection markers fall inside it.\label{fig:quality}}

A complementary view: a set of $\pm$10-second example windows (20\,s each), selected
automatically across sessions and kept away from intervals marked as noise. The
continuous trace shows the raw signal quality; the detector output is overlaid on top,
exactly as used in every analysis below: the QRS of each PVC is highlighted in
\comp{red} ($\pm$120\,ms) with a marker, while \textcolor{ForestGreen}{green}
markers tag the sinus beats. The mix includes ordinary isolated PVCs plus a
couplet, a burst and interpolated beats (tagged in each title).

\reportfigP{01_example_strips_with_pvc_detection.png}{Twenty-second windows selected automatically across sessions, detector output overlaid: PVC QRS in red, sinus beats tagged green.}

\clearpage   % forza le figure [p] di Recording quality PRIMA di Dataset overview
% ===================== DATASET OVERVIEW (dopo) =====================
\section{Dataset overview}
Across \nSessions{} sessions ($\approx$\totMinutes{} minutes, about 15 h of
single-lead ECG) the detector classified \totBeats{} heartbeats, of which
\totPVC{} were PVCs --- a cumulative burden of \cumBurden{}. A further
\exclMinutes{} of trace were marked by hand as noise and excluded from every
analysis. The burden figure is not a clinical index here; it only frames how
much ectopy this particular recorder saw over these particular recordings.

\subsection*{Sessions}
\tableSessions

% ===================== 3. PVC MORPHOLOGY =====================
\section{PVC morphology}
Each PVC is centred on its ectopic peak, normalised to its own amplitude
(peak $=1.0$), and overlaid in a common time window. Conservation of this
shape across sessions suggests a single stable ectopic focus; visible
divergences could indicate multifocality or substrate changes. The first two
panels share the same Y-axis scale for direct visual comparison.

\reportfig{02_pvc_morphology_summary.png}{PVC morphology summary: all PVCs overlaid, median by session, dispersion, and the median PVC.}

\subsection{One continuous population, or discrete subtypes?}
Visual inspection of the per-session median suggests one session shows a
slightly different hyperpolarisation profile. Is that a single focus with
continuous variability, or two morphological subtypes (mild bifocality)? Four
data-driven checks:
\begin{itemize}
...
\reportfig{03_pvc_continuum_check.png}{Single-population checks: PCA + K-means, PVCs sorted by hyperpolarisation depth, the K-means elbow, and the trough-depth histogram.}
