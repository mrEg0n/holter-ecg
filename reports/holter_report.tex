...
\textbf{Figure~\ref{fig:couplets}} summarizes the couplet analysis. The first panel reports the
number of couplets detected in each session. The second panel overlays all couplets, aligned to
the peak of the first PVC and amplitude-normalised, to assess their morphological consistency. The
third panel compares the median QRS waveform of the first PVC with that of the second PVC within
each couplet. The close overlap between the two median waveforms supports the interpretation that
the two beats in each couplet belong to the same recurrent PVC morphology.

\reportfig{13_couplets_count_and_overlay.png}{Couplet count per session, overlay of all detected couplets aligned to the first PVC peak, and comparison between the median QRS waveform of the first and second PVC within each couplet.\label{fig:couplets}}
